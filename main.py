...
async def generate_chain(req: ChainRequest):
    try:
        ...
        if character > 0.2:
            chain.append({"type": "saturation", "style": "tape", "drive": round(character * 0.5, 2), "mix": round(0.3 + character * 0.3, 2)})

        if brightness > 0.3:
            chain.append({"type": "eq_boost", "freq": 10000, "gain": round(brightness * 4, 1), "q": 0.8})

        if space > 0.1:
            chain.append({"type": "reverb", "style": "plate", "predelay": 20 + int(space * 10), "decay": round(1.0 + space * 1.5, 2), "wet": round(space * 0.35, 2)})

        chain.append({"type": "limiter", "ceiling": -1.0, "target_lufs": -14})

        return {
            "chain": chain,
            "notes": f"Applied {len(chain)}-stage chain. Detected issues: {', '.join(issues) if issues else 'none'}."
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
